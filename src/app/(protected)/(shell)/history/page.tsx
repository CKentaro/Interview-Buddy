...
function SessionRow({ s }: { s: SessionListItemResponse }) {
  ...
  return (
    <div className="ib-row" onClick={() => router.push(`/history/${s.id}`)} style={{ display: "flex", alignItems: "center", gap: 16, padding: "16px 24px", borderBottom: "1px solid var(--color-divider)", cursor: "pointer" }}>
      <div style={{ minWidth: 0, flex: 1.4, display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <div style={{ fontSize: 12, color: muted(50), fontFamily: "var(--font-jp)" }}>
          {relLabel(s.startedAt)} ・ {dateStr} ・ {stageLabel(s.selectionStage)}{s.interviewerType ? ` ・ 面接官：${s.interviewerType}` : ""}
        </div>
      </div>
